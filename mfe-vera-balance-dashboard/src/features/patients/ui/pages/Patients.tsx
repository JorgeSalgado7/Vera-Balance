...
import { EllipsisOutlined, EyeOutlined, EditOutlined, PauseCircleOutlined, PlayCircleOutlined, PlusOutlined } from '@ant-design/icons';
...
import { VBTable } from '../../../../ui/Components/VBTable';
...
import type { VBTableColumn } from '../../../../ui/Interfaces/Components/table.interface';
...
export const Patients = () => {
	...
	const handleConfirmStatusChange = () => {
		if (!patientToChangeStatus || !nextPatientStatus) return;
	...
	const getPatientActions = (patient: PatientListItem): MenuProps['items'] => {
		...
		const statusAction: NonNullable<MenuProps['items']>[number] = canReactivate
			? {
				key: 'activate',
		...
		return [
			{ key: 'record', icon: <EyeOutlined />, label: 'Ver expediente', onClick: () => navigate(ROUTES.PATIENTS.DETAIL(patient.id)) },
			{ key: 'edit', icon: <EditOutlined />, label: 'Editar paciente', onClick: () => navigate(ROUTES.PATIENTS.EDIT(patient.id)) },
			{ type: 'divider' },
			statusAction
		];
	};

	const columns: VBTableColumn<PatientListItem>[] = [
		{
			isMain: true,
			showAvatar: true,
			getLink: patient => ROUTES.PATIENTS.DETAIL(patient.id),
			column: { title: 'Nombre', dataIndex: 'name', key: 'name', width: 260 }
		},
		{
			showInMobile: true,
			column: {
		...
		},
		{
			...
			mobileRender: patient => {
				...
				return (
					<>
						<Link to={ROUTES.PATIENTS.EDIT(patient.id)}>Editar</Link>
						<span>|</span>
						<button
							type="button"
							onClick={() => handleOpenStatusModal(patient, canReactivate ? PATIENT_STATUS.ACTIVE : PATIENT_STATUS.INACTIVE)}
						>
			...
			column: {
				...
				width: 100,
				render: (_, patient) => (
					<Dropdown menu={{ items: getPatientActions(patient) }} trigger={['click']} placement="bottomRight">
						<Button type="text" shape="circle" icon={ <EllipsisOutlined /> } aria-label={`Acciones de ${patient.name}`} />
					</Dropdown>
				)
			}
		}
		...
		<DasboardLayout>
			...
			<div className="patients">
				...
				<VBCard customClass="patients__table">
					...
					<VBPageHeader title="Mis pacientes" description="Consulta y administra los expedientes de tus pacientes.">
					...
					<VBPageTools>
						<p className="patients__table__count">
							Pacientes encontrados: <strong>{ filteredPatients.length }</strong>
						</p>

						<VBSearchInput value={search} placeholder="Busca un paciente por nombre..." onChange={setSearch} />
					</VBPageTools>

					<VBTable<PatientListItem>
						rowKey="id"
						isLoading={isLoading}
