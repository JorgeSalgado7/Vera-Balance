...
interface PatientTherapyTypesProps {
	therapyTypes: TherapyType[];
}

export const PatientTherapyTypes = ({
	therapyTypes,
}: PatientTherapyTypesProps) => {
	const visibleTherapies = therapyTypes.slice(0, 2);
	const hiddenTherapies = therapyTypes.slice(2);

	const getTherapyCatalogItem = (
		therapy: TherapyType,
	) => {
		return therapyType.find(
			(catalogItem) =>
				catalogItem.value === therapy,
		);
	};
